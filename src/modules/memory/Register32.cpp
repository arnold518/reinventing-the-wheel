...
BEGIN_PINS(Register32, IOComponent)
...
void Register32::buildInternals(ComponentBuilder& builder) {
    ...
    for (size_t bit = 0; bit < 32; ++bit) {
        ...
        builder.addNewComponent<MemoryBit>(cell_name);

        builder.addNewWire(
            "D_bit_" + bit_text,
            builder.getOutputPin<BitSplitter<32>>("D_SPLIT", "OUT_" + bit_text),
            {builder.getInputPin<MemoryBit>(cell_name, "D")});

        builder.addNewWire(
            "Q_bit_" + bit_text,
            builder.getOutputPin<MemoryBit>(cell_name, "Q"),
            {builder.getInputPin<BitJoiner<32>>("Q_JOIN", "IN_" + bit_text)});

        we_sinks.push_back(builder.getInputPin<MemoryBit>(cell_name, "WE"));
        clk_sinks.push_back(builder.getInputPin<MemoryBit>(cell_name, "CLK"));
        rst_sinks.push_back(builder.getInputPin<MemoryBit>(cell_name, "RST"));
    }
