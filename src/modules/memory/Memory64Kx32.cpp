...
void requireRange(uint32_t base_address, size_t count, size_t byte_count) {
    if (count > byte_count || base_address > byte_count - count) {
        throw std::out_of_range("Memory64Kx32 address range is outside memory capacity");
    }
}

void defineMemory64Kx32Pins(IOComponent* self) {
...
const ComponentFamily Memory64Kx32{
    "rv32i.memory.64k-x32",
    "Memory64Kx32",
    defineMemory64Kx32Pins,
    nullptr,
...
Memory64Kx32::Memory64Kx32(std::string name)
    : BasicComponent(std::move(name), 1,
                     circuit::families::Memory64Kx32.pinInitializer()),
...
bool Memory64Kx32::canAccess(uint32_t address, size_t count) const {
    return count <= ByteCount && address <= ByteCount - count;
}

void Memory64Kx32::clearContents() {
...
void Memory64Kx32::loadBytes(uint32_t base_address, const std::vector<uint8_t>& data) {
...
void Memory64Kx32::loadWords(uint32_t base_address, const std::vector<uint32_t>& words) {
    if ((base_address & 0x3U) != 0) {
        throw std::invalid_argument("Memory64Kx32 word load address must be 4-byte aligned");
    }
...
std::vector<uint8_t> Memory64Kx32::readBytes(uint32_t base_address, size_t count) const {
...
uint8_t Memory64Kx32::readByte(uint32_t address) const {
    requireRange(address, 1, ByteCount);
    return byteToUInt8(bytes[static_cast<size_t>(address)]);
}

uint32_t Memory64Kx32::readWord(uint32_t address) const {
    if ((address & 0x3U) != 0) {
        throw std::invalid_argument("Memory64Kx32 word read address must be 4-byte aligned");
    }
...
uint8_t Memory64Kx32::readU8(uint32_t address) const {
    return readByte(address);
}

uint16_t Memory64Kx32::readU16(uint32_t address) const {
    const auto data = readBytes(address, 2);
    return static_cast<uint16_t>(data[0])
         | static_cast<uint16_t>(static_cast<uint16_t>(data[1]) << 8);
}

uint32_t Memory64Kx32::readU32(uint32_t address) const {
...
void Memory64Kx32::writeU16(uint32_t address, uint16_t value) {
    writeU16AtTime(0, address, value);
}

void Memory64Kx32::writeU32(uint32_t address, uint32_t value) {
    writeU32AtTime(0, address, value);
}

void Memory64Kx32::writeU8AtTime(size_t time, uint32_t address, uint8_t value) {
    requireRange(address, 1, ByteCount);
    bytes[static_cast<size_t>(address)] = byteFromUInt8(value);
    recordByteHistory(time, static_cast<size_t>(address));
}

void Memory64Kx32::writeU16AtTime(size_t time, uint32_t address, uint16_t value) {
...
void Memory64Kx32::writeU32AtTime(size_t time, uint32_t address, uint32_t value) {
    if ((address & 0x3U) != 0) {
        throw std::invalid_argument("Memory64Kx32 word write address must be 4-byte aligned");
    }
...
void Memory64Kx32::evaluate(size_t current_time, Simulator& simulator) {
...
std::vector<Memory64Kx32::MemoryWordState> Memory64Kx32::getTouchedWordsAtTime(
    size_t target_time,
    size_t max_words
) const {
...
size_t Memory64Kx32::getTouchedWordCountAtTime(size_t target_time) const {
    const auto reset = resetAtTime(target_time);
    size_t count = 0;
...
std::vector<Memory64Kx32::MemoryWordState> Memory64Kx32::getOccupiedWordsAtTime(
    size_t target_time,
    size_t max_words
) const {
...
size_t Memory64Kx32::getOccupiedWordCountAtTime(size_t target_time) const {
    return getTouchedWordCountAtTime(target_time);
}

std::vector<Memory64Kx32::MemoryWordState> Memory64Kx32::getWordsAtTime(
    size_t target_time,
...
std::map<uint32_t, uint8_t> Memory64Kx32::getByteWritesInTimeRange(
...
void Memory64Kx32::recordRangeHistory(size_t time, size_t base_address, size_t count) {
...
bool Memory64Kx32::byteTouchedAtTime(
    size_t target_time,
    size_t address,
...
bool Memory64Kx32::wordTouchedAtTime(
    size_t target_time,
    uint32_t word_index,
    const ResetHistoryEntry& reset
) const {
...
Memory64Kx32::ByteValue Memory64Kx32::byteAtTime(size_t target_time, size_t address) const {
    if (address >= ByteCount) {
        return unknownByte();
    }
...
std::vector<LogicValue> Memory64Kx32::wordAtTime(size_t target_time, uint32_t address) const {
...
void Memory64Kx32::markTrackedWord(size_t byte_address) {
