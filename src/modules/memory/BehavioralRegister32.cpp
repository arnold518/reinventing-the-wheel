...
BEGIN_PINS(BehavioralRegister32, IOComponent)
...
void BehavioralRegister32::buildInternals(ComponentBuilder& builder) {
    ...
    for (size_t bit = 0; bit < 32; ++bit) {
        ...
        builder.addNewComponent<BehavioralMemoryBit>(cell_name);

        builder.addNewWire(
            "D_bit_" + bit_text,
            builder.getOutputPin<BitSplitter<32>>("D_SPLIT", "OUT_" + bit_text),
            {builder.getInputPin<BehavioralMemoryBit>(cell_name, "D")});

        builder.addNewWire(
            "Q_bit_" + bit_text,
            builder.getOutputPin<BehavioralMemoryBit>(cell_name, "Q"),
            {builder.getInputPin<BitJoiner<32>>("Q_JOIN", "IN_" + bit_text)});

        we_sinks.push_back(builder.getInputPin<BehavioralMemoryBit>(cell_name, "WE"));
        clk_sinks.push_back(builder.getInputPin<BehavioralMemoryBit>(cell_name, "CLK"));
        rst_sinks.push_back(builder.getInputPin<BehavioralMemoryBit>(cell_name, "RST"));
    }
