...
BEGIN_PINS(Memory4x32, IOComponent)
...
void Memory4x32::buildInternals(ComponentBuilder& builder) {
    ...
    addr_sinks[2] = {
        builder.getInputPin<BitJoiner<2>>("WORD_SEL_JOIN", "IN_0"),
    };
    addr_sinks[3] = {
        builder.getInputPin<BitJoiner<2>>("WORD_SEL_JOIN", "IN_1"),
    };

    builder.addNewComponent<ORGate>("HIGH_ADDR_OR_4_5");
    addr_sinks[4].push_back(builder.getInputPin<ORGate>("HIGH_ADDR_OR_4_5", "A"));
    addr_sinks[5].push_back(builder.getInputPin<ORGate>("HIGH_ADDR_OR_4_5", "B"));
    std::string previous_high_or = "HIGH_ADDR_OR_4_5";
    for (size_t bit = 6; bit < 32; ++bit) {
    ...
    builder.addNewWire(
        ...
        {builder.getInputPin<Decoder2to4>("WRITE_DECODER", "ENABLE")});
    ...
    builder.addNewWire<2>(
        "WORD_SEL_internal",
        builder.getOutputPin<BitJoiner<2>, 2>("WORD_SEL_JOIN", "OUT"),
        {builder.getInputPin<Mux4to1_32bit, 2>("READ_MUX", "SEL"),
         builder.getInputPin<Decoder2to4, 2>("WRITE_DECODER", "ADDR")});

    builder.addNewWire<32>(
        "READ_DATA_internal",
        builder.getOutputPin<Mux4to1_32bit, 32>("READ_MUX", "OUT"),
