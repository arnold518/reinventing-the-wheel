# RV32I Decode/Control Equivalence Report
...
The focused equivalence test passes for every instruction currently supported by the RV32I answer sheet.
...
`RV32IDecodeControlUnitEquivalenceTest` runs each implementation in its own simulator with the same instruction sequence. At every row it checks each result against the independent decoder/control expectation; equality follows because both isolated traces satisfy the same complete output contract.
...
```text
ctest --test-dir build -R '^RV32IDecodeControlUnitEquivalenceTest$' --output-on-failure
RV32IDecodeControlUnitEquivalenceTest ... Passed
```
