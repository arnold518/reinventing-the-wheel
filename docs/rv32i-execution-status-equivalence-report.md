# RV32I Execution Control/Status Equivalence Report

Date: 2026-07-19

## Outcome

Block 5 has independent structural and behavioral implementations:
...
- the behavioral fidelity of `RV32IExecutionControlStatusUnit` is the compact answer sheet. It calculates the same contract directly and stores only its own halt/trap state.
- `RV32IExecutionControlStatusUnitEquivalenceTest` runs the same waveform in two isolated simulators and checks each trace against independent expected values at 30 labeled checkpoints.

The structural component does not instantiate or call the behavioral component, the instruction oracle, or `RV32IReferenceCore`.
...
Every checkpoint checks all eight output contracts against hard-coded expected values in each isolated implementation run.
...
- `rv32i-status-behavioral`: one amber the behavioral fidelity of `RV32IExecutionControlStatusUnit` root showing the compact answer-sheet contract.

The standalone scenarios use a concise eight-checkpoint teaching waveform. The 30-checkpoint `RV32IExecutionControlStatusUnitEquivalenceTest` remains in CTest as the deeper equivalence regression and is intentionally absent from the browser selector.
