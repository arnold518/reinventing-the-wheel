...
The first target is not a fully gate-level CPU. It is a structural datapath made from visible components where that is practical, with a paired behavioral implementation of each major block for fast testing and equivalence checking:

- Keep `RV32ISystem` and `BehavioralRV32ICore` as the current reference implementation.
- Add a new `RV32ISingleCycleCore` and `RV32ISingleCycleSystem`.
- Reuse the existing structural `ALU32`.
- Use the existing `RegisterFile32x32` structural shape in the educational structural core and keep `BehavioralRegisterFile32x32` as its compact counterpart.
- Continue using `BehavioralMemory64Kx32` for CPU-scale instruction and data memory; `Memory4x32`, `Memory32x32`, `Register32`, and `MemoryBit` are its lower-level teaching path.
- Give all five major core-block contracts structural and behavioral implementations with identical pins.
...
- `BehavioralRV32ICore` and `RV32ISystem`, which expose those semantics through the circuit simulator's clock, memory, and system contracts.
...
- Production structural components must not call `RV32IInstructionOracle::step`, instantiate `BehavioralRV32ICore`, or delegate architectural state updates to either one.
...
- Paired behavioral block components are permitted as component-level answer sheets and fast substitutes, but they must never be hidden inside the production structural version of the same block.
- `BehavioralMemory64Kx32` remains a permitted CPU-scale storage abstraction because it represents a separately tested memory contract; it does not decide whole-instruction RV32I semantics.
...
- `RegisterFile32x32` and `BehavioralRegisterFile32x32` already provide the structural/behavioral register-file pair with the same pins. The structural file uses compact behavioral register words internally, while the fully structural `Register32` provides the representative lower-level storage word.
- `BehavioralRegisterFile32x32` exposes register contents over time, which is useful for fast lockstep state comparison.
- `BehavioralMemory64Kx32` supports program/data loading and memory-content introspection over time.
...
- Existing behavioral program tests in `RV32ISystemTests.cpp` provide reusable RV32I program cases.
...
- `BehavioralRV32ISystemContractTest` covers reset, enable/hold, public outputs, Harvard separation, halt, and reset recovery.
...
- `RV32IExecutionControlStatusUnitPairTest`: 30 labeled state/priority checkpoints passed
- all 16 structural and all 16 behavioral program tests pass shared per-instruction lockstep and hard-coded final outcomes
- browser topology construction: all ten standalone block aliases load one structural or behavioral root each; the structural system contract loads 27,265 components and 53,761 wires
...
| Phase 1: control-flow pair | Complete | Structural and behavioral implementations share one contract and pass `RV32IControlFlowUnitPairTest`. |
| Phase 2: decode/control pair | Complete | The gate/composite decoder and behavioral answer-sheet wrapper agree for all 40 supported instructions, representative illegal encodings, and 64 deterministic raw words. |
| Phase 3A: register-file and ALU pairs | Complete for known binary CPU inputs | The direct register-file pair test and 128-row ALU pair test pass; conservative partial-unknown behavioral boundaries are documented. |
| Phase 3B: execution control/status pair | Complete | Structural gates/muxes/storage and the same-contract behavioral reference pass one shared stateful waveform. |
...
| Phase 6A: block visualizer | Complete | Ten browser scenarios keep structural and behavioral blocks separate. Structural roots are expandable; behavioral roots show the compact answer-sheet boundary. |
| Phase 6B: core/system visualizer | Complete | The structural system and core have committed hierarchy layouts and load as a 27,265-component browser topology. |
...
The first structural single-cycle CPU milestone and its first lossless browser-performance pass are implemented. The next work is hardening rather than missing core wiring: external Spike/Sail or architectural-suite differential validation, an optional deliberate nonzero reset-vector contract, and conservative cold-start/topology-sharing improvements for the 27,000-component hierarchy.

`RV32IExecutionControlStatusUnit` now centralizes the decisions that the first four blocks deliberately do not own: whether the current instruction may update PC or a register, whether a memory request stays active while waiting, which halt/trap state latches, and which fault wins when several requests are present together. Its detailed implementation and verification record is in `docs/rv32i-execution-status-pair-report.md`.
...
Each contract gets two independent implementations:
...
- The behavioral implementation is a compact, clearly named `BasicComponent` reference/bring-up tool. It has the same external pins and must not instantiate, call, or read private state from the structural implementation. It is not a product-facing default; promote it to a fast configuration only if measured structural cost justifies that use after equivalence is proven.

| Block contract | Structural implementation | Behavioral counterpart | Current state |
| --- | --- | --- | --- |
| Control flow | `RV32IControlFlowUnit`: `Register32`, `PC+4`/target adders, JALR mask, branch gates, next-PC mux, alignment check | `BehavioralRV32IControlFlowUnit`: direct PC/target/branch calculation with the same clocked contract | Complete; focused pair test passes |
| Decode/control | `RV32IDecodeControlUnit`: field rewires, I/S/B/U/J immediate wiring, instruction-recognition gates, and control-output gates | `BehavioralRV32IDecodeControlUnit`: maps known instructions through `RV32IDecoder`/`RV32IControl` | Complete for known 32-bit instructions, including 64 deterministic raw words; partial-unknown boundary documented |
| Register file | `RegisterFile32x32`: visible decoder/read muxes/register-word hierarchy | `BehavioralRegisterFile32x32`: compact direct state | Complete; direct pair test exercises x0 and every x1-x31 entry |
| ALU | `ALU32`: existing add/subtract, logic, shifter, comparator, and zero-detect composites | `BehavioralALU32`: direct operation/flag calculation using the identical pins and op encoding | Complete for known binary inputs; conservative partial-unknown boundary documented |
| Execution control/status | `RV32IExecutionControlStatusUnit`: alignment gates, status storage, trap-priority muxing, and permission gates | `BehavioralRV32IExecutionControlStatusUnit`: direct state machine and priority calculation with the same pins | Complete; stateful priority/handshake pair test passes |
...
`BehavioralMemory64Kx32` already selects byte/halfword/word lanes, sign-extends loads, checks alignment/range, reports ready/fault, rejects faulting stores, and records actual byte writes. The only CPU-specific memory logic is low-bit address-alignment classification so the correct load/store trap cause can be latched; that small logic belongs in `RV32IExecutionControlStatusUnit`.

`RV32ISingleCycleCore` and `RV32ISingleCycleSystem` are hierarchy containers, not two more execution units:

- `RV32ISingleCycleCore` wires the structural member of each of the five pairs, generic muxes, and direct memory-interface signals.
- `RV32ISingleCycleSystem` contains the core plus instruction and data memory.

The current monolithic `BehavioralRV32ICore` remains the independent program-level answer sheet. It does not count as the five component-level behavioral counterparts because it does not expose their individual pin contracts.

### Pairing and equivalence rules

The dual implementation strategy is useful only if the two sides remain genuinely independent:

1. Freeze one pin contract and one encoding table for the pair.
2. Build and test the structural implementation or representative lower-level slice first.
3. Add the behavioral component only after the lower-level contract is tested.
4. Run the same directed and randomized inputs through both components.
...
8. Keep `RV32IInstructionOracle` and `BehavioralRV32ICore` out of every structural component's production dependencies.
...
This contract already exists in both `RegisterFile32x32` and `BehavioralRegisterFile32x32`. The core connects final—not raw—register-write permission to `REG_WRITE`.
...
This contract already exists in structural `ALU32`. `BehavioralALU32` must copy these pins and the `ALU32Op` encoding exactly. Generic core muxes select `A` and `B`.
...
- paired register storage: `RegisterFile32x32`, `BehavioralRegisterFile32x32`
- scale memory: `BehavioralMemory64Kx32`, backed educationally by the existing smaller structural storage path
...
- `RV32IInstructionOracle` and `BehavioralRV32ICore`: whole-instruction/program answer sheet only
- `BehavioralRV32IControlFlowUnit`, `BehavioralRV32IDecodeControlUnit`, `BehavioralALU32`, and `BehavioralRV32IExecutionControlStatusUnit`: same-contract component references and optional fast substitutes, never children of their structural counterparts
...
- Data memory from `BehavioralMemory64Kx32`.
...
- `DMEM_WRITE_EN` is a store request, not proof that memory changed. It may be asserted while the memory reports a fault; `BehavioralMemory64Kx32` must then reject the request and record no byte-write transaction.
...
### Phase 1: Paired Control-Flow Unit

Status: complete. See `docs/rv32i-control-flow-pair-report.md`.

Add structural `RV32IControlFlowUnit` as an expandable composite. After its structural tests pass, add compact `BehavioralRV32IControlFlowUnit` with exactly the same pins.
...
Build the field and immediate paths structurally from bit adapters, rewires, constants, muxes, and gates. Build instruction-recognition terms and combine them into the raw control outputs. The behavioral counterpart may use `RV32IDecoder` and `RV32IControl` for known 32-bit inputs, but it remains a separate component under test; it is not an internal bridge inside `RV32IDecodeControlUnit`.
...
### Phase 3: Complete All Five Block Pairs

Status: complete. See `docs/rv32i-register-file-pair-report.md`, `docs/rv32i-alu-pair-report.md`, and `docs/rv32i-execution-status-pair-report.md`.

Before core integration, complete the two pairs whose structural members already exist:

- Add `BehavioralALU32` with the exact `ALU32` pins, operation encoding, output flags, and defined unknown-value behavior.
...
- `RV32IRegisterFilePairTest` now drives both implementations from the same reset/read/write waveform, selects every architectural entry through both read paths, and compares the externally observable architectural values directly.
...
`BehavioralRV32IExecutionControlStatusUnit` has the identical pins and state semantics and is checked directly against the structural unit.
...
- `BehavioralMemory64Kx32` named `INSTRUCTION_MEMORY`
- `BehavioralMemory64Kx32` named `DATA_MEMORY`
...
System helper APIs should mirror the current behavioral `RV32ISystem` where practical:
...
- Mirror `BehavioralRV32ISystemContractTest` for reset, enable/hold, public PC/status pins, Harvard separation, halt, ignored clocks after halt/trap, and reset recovery.
...
- Done: add separate structural and behavioral aliases for control flow, decode, register file, ALU, and execution status. Each scenario owns one device under test; pair equivalence tests are intentionally excluded from the browser selector.
...
- All five major block contracts have structural and behavioral implementations, with explicit pairwise equivalence tests passing.
...
`ALU32` and `RegisterFile32x32` are structurally large. The single-cycle system may be much slower than the behavioral system, so early tests should be short and timing should be conservative. The behavioral counterparts make isolated fast testing possible, but passing a fast behavioral assembly is not evidence that the structural assembly works.
