# RV32I ALU Equivalence Report

## Outcome

Block 4 now has two implementations of the same contract:

- `ALU32`: the existing structural implementation.
- the behavioral fidelity of `ALU32`: the new compact reference/fast substitute.

`ALU32EquivalenceTest` passes all known-value equivalence rows and the behavioral unknown-input contract.
...
the behavioral fidelity of `ALU32` calculates the same known binary results directly in one compact component. It is a sibling used as an answer sheet or optional fast substitute; it is not hidden inside `ALU32`.
...
The equivalence test runs each component in its own simulator with the same vectors and checks both against an independent expected-value function. Its 128 known-value rows cover:
...
```text
ctest --test-dir build -R '^ALU32EquivalenceTest$' --output-on-failure
ALU32EquivalenceTest ... Passed
```
