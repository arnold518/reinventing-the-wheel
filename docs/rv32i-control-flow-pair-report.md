# RV32I Control-Flow Pair Report

Last updated: 2026-07-19

## Status

Block 1 is implemented and its focused pair test passes.

Implemented components:

- structural `RV32IControlFlowUnit`
- behavioral reference `BehavioralRV32IControlFlowUnit`

Both components expose the same pins and are driven together by `RV32IControlFlowUnitPairTest`.
...
`BehavioralRV32IControlFlowUnit` owns only its compact PC state and direct control-flow calculation. It is not instantiated inside the structural component.
...
```bash
ctest --test-dir build -R '^RV32IControlFlowUnitPairTest$' --output-on-failure
```
