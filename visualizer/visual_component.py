...
class VisualPin:
    def __init__(self, parent_component, cpp_pin_handle, pin_type: str):
        ...
        self.rect = pygame.Rect(0, 0, 1, 1) # Placeholder, will be resized by layout
        self.color = STATE_COLORS[circuit_backend.LogicValue.UNKNOWN]
        self.text_color = (220, 220, 220)
        self.base_points = [] # Placeholder, will be calculated by layout
...
class VisualWire:
    ...
    def connect(self, pin_map):
        ...
        if source_cpp and (owner := source_cpp.get_owner()):
            key = f"{owner.get_name()}.{source_cpp.get_name()}"
            self.source_vpin = pin_map.get(key)
        for sink_cpp in self.cpp_handle.get_sink_pins():
            if (owner := sink_cpp.get_owner()) and (vpin := pin_map.get(f"{owner.get_name()}.{sink_cpp.get_name()}")):
    ...
    def collidepoint(self, world_pos, camera):
        ...
        for path in self._world_paths:
            for i in range(len(path) - 1):
                ...
                if (world_pos - closest_point).length_squared() < threshold ** 2: return True
        return False
...
class VisualComponent:
    ...
    def __init__(self, rect, cpp_handle, settings, depth=0, parent=None, color=(61, 90, 128, 100)):
        self.cpp_handle = cpp_handle
        self.parent, self.children = parent, []
        self.rect = rect
        self.is_dragging, self._is_hovered = False, False
    ...
    def _render_surfaces(self, zoom):
        zoomed_size = (max(1, self.rect.width * zoom), max(1, self.rect.height * zoom))
        zoomed_title_height = zoomed_size[0] * self.title_bar_ratio
    ...
    def draw(self, screen, camera):
        ...
        screen.blit(self._body_surface, (screen_pos.x, screen_pos.y + zoomed_title_height))
    ...
    def handle_event(self, event, camera):
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and self.is_hovered:
            self.is_dragging = True; self.drag_offset = camera.screen_to_world(pygame.mouse.get_pos()) - self.rect.topleft; return True
        if event.type == pygame.MOUSEBUTTONUP and event.button == 1 and self.is_dragging:
            self.is_dragging = False; return True
        if event.type == pygame.MOUSEMOTION and self.is_dragging:
            new_pos = camera.screen_to_world(pygame.mouse.get_pos()) - self.drag_offset; self.move(new_pos - self.rect.topleft); return True
        return False
    def move(self, delta):
...
class VisualIOComponent(VisualComponent):
    ...
    def _layout_pins(self):
        ...
        pin_width = pin_height # Keep it simple, 1:1 aspect ratio for the pin shape

        if self.input_pins:
            spacing = content_rect.height / (len(self.input_pins) + 1)
            for i, pin in enumerate(self.input_pins):
                # --- NEW: Set proportional size and points ---
                pin.rect.size = (pin_width, pin_height)
                h2, w2 = pin.rect.height / 2, pin.rect.width / 2
