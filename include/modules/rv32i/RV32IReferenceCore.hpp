...
extern const ComponentFamily RV32IReferenceCore;
}

class RV32IReferenceCore : public BasicComponent {
public:
    RV32IReferenceCore(std::string name);
    static constexpr const char* TypeName = "RV32IReferenceCore";
    const char* getTypeName() const override { return TypeName; }
    void evaluate(size_t current_time, Simulator& simulator) override;

    void attachMemories(std::shared_ptr<Memory64Kx32> instruction_memory,
...
private:
    ...
    std::weak_ptr<Memory64Kx32> instruction_memory_;
    std::weak_ptr<Memory64Kx32> data_memory_;
